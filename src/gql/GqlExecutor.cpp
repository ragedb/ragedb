...
static std::optional<StarJoinCandidate> find_star_join_candidate(
 ...
 */
seastar::future<IntermediateResult> execute_match_chain_factorized(ragedb::Graph& graph, std::vector<MatchStatement> matches, size_t match_idx, IntermediateResult incoming, size_t limit = 0, const ProjectionPruner& pruner = {}, std::string sort_property = "", bool sort_ascending = true, bool sort_by_id = false) {
    ...
    if (auto candidate = find_star_join_candidate(matches, match_idx, incoming_vars)) {
        ...
        if (incoming_vars.count(central_var)) {
            // Case A: central_var is already bound in incoming.
            // Execute each branch in S independently relative to incoming, then join them factorized.
            ...
            // Case B: central_var is not yet bound in incoming.
            // Execute the first branch in S to bind central_var, then execute the rest factorized.
