...
// Helper: Run CPU-bound work in standard background thread
template <typename Func>
auto run_background_task(Func&& func) {
    using ResultType = typename std::invoke_result_t<Func>;
    auto* prom = new seastar::promise<ResultType>();
    ...
    std::async(std::launch::async, [prom, shard_id, func = std::forward<Func>(func)]() mutable {
        try {
            ...
            seastar::alien::run_on(*seastar::alien::internal::default_instance, shard_id, [prom, ex = std::move(ex)]() mutable noexcept {
                prom->set_exception(std::move(ex));
                delete prom;
            });
        }
    });
...
seastar::future<IntermediateResult> HoneycombExecutor::execute(
    ragedb::Graph& graph,
    ...
    .then([&graph, rel_metas, matches, limit, node_vars, edge_vars](std::vector<std::vector<Relationship>> all_rels) {
        ...
        return run_background_task([relations = std::move(relations), global_vars, limit, edge_variables_set]() mutable {
        ...
        .then([&graph, global_vars, edge_variables_set](std::vector<std::vector<uint64_t>> results_ids) {
            ...
            .then([results_ids = std::move(results_ids), global_vars, var_is_node, node_ids, rel_ids](auto tuple) {
                ...
                for (const auto& row_ids : results_ids) {
                    ...
                    for (size_t i = 0; i < global_vars.size(); ++i) {
                        ...
                        // Avoid inserting anonymous node or edge variables if we don't want them in output, 
                        // but actually keeping them is safe and helpful for downstream filters.
                        if (var_is_node[i]) {
