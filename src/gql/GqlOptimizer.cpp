...
void GqlOptimizer::optimize(GqlQuery& query) {
    ...
    // Run cardinality limit pass
    semantic_cardinality_limit_pass(query);
...
static std::vector<CardinalityConstraint> find_cardinality_constraints() {
    ...
    for (const auto& [name, query_str] : constraints) {
        try {
            ...
            // We expect at least two matches (N >= 2)
            ...
            for (size_t i = 0; i < N; ++i) {
                ...
                // Edge direction and label check
            ...
            // Now collect inequalities from the WHERE expression
            ...
            // Check that all pairs of target variables are mutually distinct
            ...
            if (all_distinct) {
                // If N variables are mutually distinct, max cardinality is N - 1
...
/**
 * @brief Phase 5: Cardinality-Constrained Traversal Short-Circuiting.
 *
 * Mathematical Axioms Applied: Cardinality bounds on functions and relations.
 *
 * If the database catalog has a constraint: e.g. Person has FRIEND out-degree <= 1.
 * The physical executor can immediately short-circuit edge scanning for any starting node as soon as C (in this case, 1) matching relationship is traversed, bypassing the remaining shard list lookup.
 * This avoids allocating dynamic arrays for neighbors, replacing them with a single pointer.
 */
void GqlOptimizer::semantic_cardinality_limit_pass(GqlQuery& query) {
    ...
    for (auto& match : query.matches) {
        ...
        for (size_t i = 0; i < pattern.edges.size(); ++i) {
            ...
            // We can only check if the source node has a label and the relationship has a type.
            ...
            // Map EdgeDirection to Direction
            Direction dir = Direction::BOTH;
            if (edge.direction == EdgeDirection::RIGHT) {
                dir = Direction::OUT;
            } else if (edge.direction == EdgeDirection::LEFT) {
