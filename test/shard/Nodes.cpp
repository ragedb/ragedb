    ...
    GIVEN("A shard with an empty node and an existing node with properties") {
        ...
        WHEN("a node with properties is added") {
            ...
            THEN("the shard keeps it with properties") {
                ...
                REQUIRE("max de marzi" == std::any_cast<std::string>(shard.NodePropertyGet(added, "name")));
                REQUIRE("maxdemarzi@gmail.com" == std::any_cast<std::string>(shard.NodePropertyGet(added, "email")));
            }
        }
