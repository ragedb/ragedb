...
SCENARIO( "Shard can execute graph Similarity algorithms", "[graph_similarity]" ) {
    GIVEN("A graph with 4 nodes forming a cycle") {
        auto graph = Graph("graph_similarity_test");
        graph.Start().get();
        graph.Clear();
        ...
        WHEN("We query C++ similarity algorithms directly") {
            
            THEN("JaccardSimilarity computes scores") {
                auto jaccard = graph.shard.local().JaccardSimilarityPeered("KNOWS", "", "", "", "", false, false);
                REQUIRE_FALSE(jaccard.empty());
            }

            THEN("CosineSimilarity computes scores") {
                auto cosine = graph.shard.local().CosineSimilarityPeered("KNOWS", "", "", "", "", false, false);
                REQUIRE_FALSE(cosine.empty());
            }

            THEN("AdamicAdar computes scores") {
                auto adamic = graph.shard.local().AdamicAdarPeered("KNOWS", "", "", "", "", false, false);
                REQUIRE_FALSE(adamic.empty());
            }

            THEN("PreferentialAttachment computes scores") {
                auto pref = graph.shard.local().PreferentialAttachmentPeered("KNOWS", "", "", "", "", false, false);
                REQUIRE_FALSE(pref.empty());
            }
        }

        WHEN("We query via Lua graph similarity bindings") {
            
            THEN("Lua JaccardSimilarity executes") {
                std::string script = "local res = JaccardSimilarity('KNOWS', '', '', '', '', false, false)\n"
                                     "#res > 0";
