...
TEST_CASE("Edge Contradiction Performance Benchmarks", "[gql_optimizer_perf]") {
    auto graph = Graph("gql_test_edge_contradiction_perf");
    graph.Start().get();
    graph.Clear();
    ...
    std::cout << "\n=========================================\n";
    std::cout << "   EDGE CONTRADICTION PERFORMANCE BENCH\n";
    std::cout << "=========================================\n";
    ...
    run_bench(
        graph,
        "Phase 17: Relationship Contradiction Pruning",
        "MATCH (a:Person)-[r:FRIEND]->(b:Person) WHERE r.weight = -5 RETURN a.name",
        "NO_SEMANTIC MATCH (a:Person)-[r:FRIEND]->(b:Person) WHERE r.weight = -5 RETURN a.name",
        2
    );
