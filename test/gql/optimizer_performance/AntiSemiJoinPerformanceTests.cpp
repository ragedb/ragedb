...
TEST_CASE("Anti-Semi-Join Performance Benchmarks", "[gql_optimizer_perf]") {
    auto graph = Graph("gql_test_anti_semi_join_perf");
    graph.Start().get();
    graph.Clear();
    ...
    std::cout << "\n=========================================\n";
    std::cout << "   ANTI-SEMI-JOIN PERFORMANCE BENCH\n";
    std::cout << "=========================================\n";

    run_bench(
        graph,
        "Phase 18: Anti-Semi-Join Promotion",
        "OPTIONAL MATCH (p:Person)-[:FRIEND]->(f:Person) WHERE f IS NULL RETURN p.name",
        "NO_SEMANTIC OPTIONAL MATCH (p:Person)-[:FRIEND]->(f:Person) WHERE f IS NULL RETURN p.name",
        2
    );
