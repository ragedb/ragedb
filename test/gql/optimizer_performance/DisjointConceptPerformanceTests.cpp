...
TEST_CASE("Disjoint Concept Performance Benchmarks", "[gql_optimizer_perf]") {
    auto graph = Graph("gql_test_disjoint_concept_perf");
    graph.Start().get();
    graph.Clear();
    ...
    std::cout << "\n=========================================\n";
    std::cout << "   DISJOINT CONCEPT PERFORMANCE BENCH\n";
    std::cout << "=========================================\n";
    ...
    run_bench(
        ...
        "MATCH (a:Category {name: 'SciFi'})-[:SUB_CATEGORY*]->(b:Category {name: 'Biography'}) RETURN a.name",
        "NO_SEMANTIC MATCH (a:Category {name: 'SciFi'})-[:SUB_CATEGORY*]->(b:Category {name: 'Biography'}) RETURN a.name",
        2
    );
