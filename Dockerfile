...
RUN apt install -y build-essential git sudo pkg-config ccache python3-pip \
    valgrind libfmt-dev gcc-12 g++-12 ninja-build ragel libhwloc-dev libnuma-dev libpciaccess-dev libcrypto++-dev libboost-all-dev \
...
RUN pip install --user conan -v "conan>=2.0"
...
RUN ./configure.py --mode=release --prefix=/usr/local --compiler=g++-12 --c-compiler=gcc-12 --without-tests --without-apps --without-demos
